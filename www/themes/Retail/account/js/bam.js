...
/*
 * Array unique value
 * Extending jQuery’s $.unique() rather than extending the native JavaScript Array() via prototype.
...
var ChargebackCall = {
	...
	getChargeback: function(regionNumber, region) {
		$.ajax({
			timeout: 60000,
			url: Core.staticUrl + '/data/wow-license-details.html?region=' + region + '&_=' + new Date().getTime(),
	...
	showChargeback: function(hasCookie) {
		var display = ($.browser.msie && ($.browser.version == "6.0" || $.browser.version == "7.0")) ? 'inline' : 'inline-block'; // IE doesn’t support inline-block on block-level elements (except 8).
		if (ChargebackCall.chargebackCount > 0 ) {
			this.target.text(ChargebackCall.chargebackCount).css("display", display);
		}
...
var accountBalance = {
	...
	changeCurrencyMenuFlag: false,
	currencyMap: [[],[]],

	initialize: function() {
		if (accountBalancePrimary){
			accountBalance.setPrimary(accountBalancePrimary);
		}else{
			accountBalance.getAccountBalancePrimary();
		}
		...
		this.accountBalanceMenu.find(".switch-currency").each(function(){
			$(this).bind({
				'click': function() {
					...
					if(!flagSelected){
						accountBalancePrimary = $(this).attr("id");
						accountBalance.setAccountBalancePrimary();
					}
				}
			});
		});
	...
	getAccountBalancePrimary: function() {
		$.ajax({
			timeout: 60000,
			url: Core.staticUrl + '/data/get-account-cookie.html?prefix=CUR&_=' + new Date().getTime(),
			success: function(msg) {
				if (msg !== '' && msg.successful !== '') {
					if (msg.successful === true && msg.value !== undefined) {
						//checking currency
						for (var i = 0, l = accountBalance.currencyMap[0].length; i < l; i++) {
							if (msg.value == accountBalance.currencyMap[0][i]) {
								accountBalancePrimary = msg.value;
								break;
							}else{
								accountBalancePrimary = accountBalanceDefaultCurrency;
							}
						}
					}else{
						accountBalancePrimary = accountBalanceDefaultCurrency;
					...
					for (var j=0, l = accountBalance.currencyMap[0].length; j < l; j++) {
						if (accountBalancePrimary == accountBalance.currencyMap[0][j]) {
							accountBalanceFormat = accountBalance.currencyMap[1][j];
						}
					}
	...
	setPrimary: function(value) {
		...
		accountBalancePrimary = value;

		if (accountBalancePrimary == "BET") {
			$(".nonBeta").hide();
		}else{
			$(".nonBeta").show();
		}
		...
		if (primaryCurrency !== null) {
			...
			price = 0.00;
			this.accountBalancePrimaryBalance.html(accountBalanceFormat.replace("@", price));
		}
	},

	setAccountBalancePrimary: function(selected) {

		if (accountBalancePrimary !== null && accountBalancePrimary !== '' && accountBalancePrimary !== 'undefined' && accountBalancePrimary !== undefined) {
			if (selected) {
				accountBalancePrimary = selected;
			}
			$.ajax({
				timeout: 60000,
				url: Core.staticUrl + '/data/set-account-cookie.html?prefix=CUR&value=' + accountBalancePrimary + '&_=' + new Date().getTime(),
				success: function(msg) {
					if (msg !== '' && msg.successful !== '') {
						if (msg.successful === true && msg.value !== undefined) {
