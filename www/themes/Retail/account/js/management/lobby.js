...
var Lobby = {
    ...
    toggleGameList: function(node, target) {
        ...
        if (targetElement.is(':visible')) {
			...
			Cookie.erase('bam.' + target, 'closed');
    ...
    getAccountDetails: function(unloaded, region) {
        $.ajax({
			timeout: 60000,
            url: Core.staticUrl + '/data/wow-licenses-details.html?region=' + region + '&_=' + new Date().getTime(),
			beforeSend: function() {
				$(unloaded).removeClass('unloaded').addClass('loading');
				// D3 for WoW
				$('#d3-account-link').addClass('loading');
				$('#d3-community-link').addClass('loading');
				$('#d3-promotion-link').addClass('loading');
			},
            success: function(msg) {
            	// D3 for WoW
				$('#d3-account-link').removeClass('loading');
				$('#d3-community-link').removeClass('loading');
				$('#d3-promotion-link').removeClass('loading');
				for (var i = 0, length = msg.length; i < length; i++) {
					if (msg[i] !== '' && msg[i].status !== 'ERROR') {
						...
						// D3 for WoW
						if (message.promoContract) {
							$('#d3-account-link').show();
							$('#d3-community-link').hide();
							$('#d3-promotion-link').hide();
						} else if (Promotion.available && message.promoEligible && !$('#d3-account-link').is(':visible')) {
							$('#d3-community-link').hide();
