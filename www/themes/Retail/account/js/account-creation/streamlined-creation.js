...
var Creation = Class.extend({
	...
	battleTagInput1: null,
	battleTagMessage1: null,
	battleTagTimer: null,
	battleTagGuidelines: null,
	showbattleTagGuidelines: false,
	...
	setup: function() {
		...
		this.battleTagInput1 = $(this.config.battleTagFields[0]);
		this.battleTagMessage1 = $(this.config.battleTagFields[0] + '-message');
		this.battleTagGuidelines = $('#battlenet-id-note');
		...
		this.battleTagInput1.bind('focus blur input',
			$.proxy(this._togglebattleTagNote, this)
		);
	...
	/**
	 * Internal call to togglebattleTagNote() that uses a timer as a limiter.
	 *
	 * @param e The event data.
	 */
	_togglebattleTagNote: function(e) {
		var type = e.type,
			bind = this;

		this.showbattleTagGuidelines = type !== 'blur';
		if (this.battleTagTimer === null) {
			this.battleTagTimer = setTimeout(function() {
				bind.togglebattleTagNote();
			}, 0);
			return true;
		}
		return false;
	...
	/**
	 * Toggle the battlenet Id information.
	 */
	togglebattleTagNote: function() {

		if (this.showbattleTagGuidelines) {
			this.battleTagGuidelines.show();
		} else {
			this.battleTagGuidelines.hide();
		}

		if (this.battleTagTimer !== null) {
			clearTimeout(this.battleTagTimer);
			this.battleTagTimer = null;
		}
